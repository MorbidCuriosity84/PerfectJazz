...
struct playerSettings {
	...
	playerSettings(int dam, int hp, double ft, Scene* scene, float res, float fr, sf::Vector2f vel, _entityCategory cat, bool hpV, sf::Vector2f wScale, sf::Vector2f sScale, _entityCategory wepCat)
		: _damage(dam), _hp(hp), _fireTime(ft), _wepSpriteScale(sScale), _spriteScale(sScale), _scene(scene), _restitution(res), _friction(fr), _velocity(vel), _cat(cat), _hpVisible(hpV), _wepCat(wepCat) {
	}
};
